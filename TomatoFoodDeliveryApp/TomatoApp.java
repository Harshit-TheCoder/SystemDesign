package TomatoFoodDeliveryApp;


import java.util.List;

import TomatoFoodDeliveryApp.factories.NowOrderFactory;
import TomatoFoodDeliveryApp.factories.OrderFactory;
import TomatoFoodDeliveryApp.factories.ScheduledOrderFactory;
import TomatoFoodDeliveryApp.managers.OrderManager;
import TomatoFoodDeliveryApp.managers.RestuarentManager;
import TomatoFoodDeliveryApp.models.Cart;
import TomatoFoodDeliveryApp.models.MenuItem;
import TomatoFoodDeliveryApp.models.Order;
import TomatoFoodDeliveryApp.models.Restuarent;
import TomatoFoodDeliveryApp.models.User;
import TomatoFoodDeliveryApp.services.NotificationService;
import TomatoFoodDeliveryApp.strategies.PaymentStrategy;


public class TomatoApp {

    public TomatoApp() {
        initializeRestuarents();
    }

    public void initializeRestuarents() {
        Restuarent restaurant1 = new Restuarent("Bikaner", "Delhi");
        restaurant1.addMenuItem(new MenuItem("P1", "Chole Bhature", 120));
        restaurant1.addMenuItem(new MenuItem("P2", "Samosa", 15));

        Restuarent restaurant2 = new Restuarent("Haldiram", "Kolkata");
        restaurant2.addMenuItem(new MenuItem("P1", "Raj Kachori", 80));
        restaurant2.addMenuItem(new MenuItem("P2", "Pav Bhaji", 100));
        restaurant2.addMenuItem(new MenuItem("P3", "Dhokla", 50));

        Restuarent restaurant3 = new Restuarent("Saravana Bhavan", "Chennai");
        restaurant3.addMenuItem(new MenuItem("P1", "Masala Dosa", 90));
        restaurant3.addMenuItem(new MenuItem("P2", "Idli Vada", 60));
        restaurant3.addMenuItem(new MenuItem("P3", "Filter Coffee", 30));

        RestuarentManager restaurantManager = RestuarentManager.getInstance();
        restaurantManager.addRestuarent(restaurant1);
        restaurantManager.addRestuarent(restaurant2);
        restaurantManager.addRestuarent(restaurant3);
    }

    public List<Restuarent> searchRestuarents(String location) {
        return RestuarentManager.getInstance().searchByLocation(location);
    }

    public void selectRestuarent(User user, Restuarent restaurant) {
        Cart cart = user.getCart();
        cart.setRestuarent(restaurant);
    }

    public void addToCart(User user, String itemCode) {
        Restuarent restaurant = user.getCart().getRestuarent();
        if (restaurant == null) {
            System.out.println("Please select a restaurant first.");
            return;
        }
        for (MenuItem item : restaurant.getMenu()) {
            if (item.getCode().equals(itemCode)) {
                user.getCart().addItem(item);
                break;
            }
        }
    }

    public Order checkoutNow(User user, String orderType, PaymentStrategy paymentStrategy) {
        return checkout(user, orderType, paymentStrategy, new NowOrderFactory());
    }

    public Order checkoutScheduled(User user, String orderType, PaymentStrategy paymentStrategy, String scheduleTime) {
        return checkout(user, orderType, paymentStrategy, new ScheduledOrderFactory(scheduleTime));
    }

    public Order checkout(User user, String orderType, PaymentStrategy paymentStrategy, OrderFactory orderFactory) {
        if (user.getCart().isEmpty()) return null;

        Cart userCart = user.getCart();
        Restuarent orderedRestuarent = userCart.getRestuarent();
        List<MenuItem> itemsOrdered = userCart.getItems();
        double totalCost = userCart.getTotalCost();

        Order order = orderFactory.createOrder(user, userCart, orderedRestuarent, itemsOrdered, paymentStrategy, totalCost, orderType);
        OrderManager.getInstance().addOrder(order);
        return order;
    }

    public void payForOrder(User user, Order order) {
        boolean isPaymentSuccess = order.processPayment();

        if (isPaymentSuccess) {
            NotificationService.notify(order);
            user.getCart().clear();
        }
    }

    public void printUserCart(User user) {
        System.out.println("Items in cart:");
        System.out.println("------------------------------------");
        for (MenuItem item : user.getCart().getItems()) {
            System.out.println(item.getCode() + " : " + item.getName() + " : ₹" + item.getPrice());
        }
        System.out.println("------------------------------------");
        System.out.println("Grand total : ₹" + user.getCart().getTotalCost());
    }
}
